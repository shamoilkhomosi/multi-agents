function dzdt = maspde_qua(t, z, x, phiL, phiR, Na, h, Nh, k, a, v, kr, r)
...
%         z_soh(j) = polyval(coeff, j);
        ...
        z_soh(j) = (x_lead-j)/(x_lead-x_lead_l)*z_soh_0 + (j-x_lead_l)/(x_lead-x_lead_l)*z_soh_1;
...
%         z_soh(j) = polyval(coeff, j);
        ...
        z_soh(j) = (x_lead_r-j)/(x_lead_r-x_lead)*z_soh_0 + (j-x_lead)/(x_lead_r-x_lead)*z_soh_1;
